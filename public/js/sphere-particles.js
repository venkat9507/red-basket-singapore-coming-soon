  ...
  var palette = [
   ...
   function rgbString() {
     ...
     if(Math.floor(Math.random()*50) != 1) 
       return "rgba("+palette[0].r+","+palette[0].g+","+palette[0].b+",";
    ...
    y = 0; //try changing to a positive number (not too large, for example 0.3), or negative for floating upwards.

		particleRad = 11;

		sphereCenterX = 0;
		sphereCenterY = 0;
		sphereCenterZ = -3 - radiusofSphere;
	...
	function animateParticle() {
		...
		x0 = radiusofSphere*Math.sin(phi)*Math.cos(theta);
		y0 = radiusofSphere*Math.sin(phi)*Math.sin(theta);
		z0 = radiusofSphere*Math.cos(phi);
...
setTimeout(function(){
  $('canvas').attr('height',$('div').width());
  canvasApp();
}, 500);

$(window).resize(function(){
  $('canvas').attr('height',$('div').height()-$('textarea').height());
});